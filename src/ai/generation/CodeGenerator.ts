...
/** Audits generated bytes against backend-canonicalized authority-zero topology. */
export function findVerifiedTopologyIssues(
  sourceMap: Readonly<Record<string, string>>,
  changes: readonly AgentFileChange[],
  manifest?: FileManifest | null,
): string[] {
  const graph = manifest?.verifiedTopology;
  if (!graph) return [];
  const snapshotFiles = Object.entries(sourceMap).map(([filePath, content]) => ({ path: normalizeRepoPath(filePath), content }));
  const validation = StaticValidationEngine.validate(snapshotFiles, [...changes]);
  const dependencies = new Map<string, Set<string>>();
  for (const [source, targets] of validation.dependencyGraph || new Map<string, string[]>()) {
    dependencies.set(normalizeRepoPath(source), new Set(targets.map(normalizeRepoPath)));
  }
  const postChange = new Map(snapshotFiles.map((file) => [normalizeRepoPath(file.path), file.content]));
  for (const change of changes) {
    const normalized = normalizeRepoPath(change.path);
    if (change.action === "delete" || change.isDeleted) postChange.delete(normalized);
    else postChange.set(normalized, change.content);
  }
  ...
  for (const edge of graph.edges) {
    ...
    if (!source || !target) {
      issues.push("Canonical topology edge references a missing node");
      continue;
    }
    if (edge.relation === "IMPORTS" || edge.relation === "DEPENDS_ON") {
      if (!dependencies.get(normalizeRepoPath(source.path))?.has(normalizeRepoPath(target.path))) {
        issues.push(`${source.path} does not resolve its ${edge.relation} edge to ${target.path}`);
      }
    } else if (edge.relation === "RENDERS") {
      const symbol = target.symbol || path.posix.basename(target.path, path.posix.extname(target.path));
      const escaped = symbol.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const importsTarget = dependencies.get(normalizeRepoPath(source.path))?.has(normalizeRepoPath(target.path));
      if (!importsTarget || !new RegExp(`<${escaped}(?:\\s|/|>)`).test(postChange.get(normalizeRepoPath(source.path)) || "")) {
        issues.push(`${source.path} does not render ${symbol} from ${target.path}`);
      }
    } else if (edge.relation === "REGISTERS") {
      const content = postChange.get(normalizeRepoPath(source.path)) || "";
      const importsTarget = dependencies.get(normalizeRepoPath(source.path))?.has(normalizeRepoPath(target.path));
      if (!importsTarget || !/\.(?:use|register|route)\s*\(/.test(content)) {
        issues.push(`${source.path} does not deterministically register ${target.path}`);
      }
    } else if (edge.relation === "ROUTES_TO" && target.role === "ROUTE" && !describeFrameworkRoute(target.path)) {
      issues.push(`${target.path} is not a valid framework route`);
    }
  }
  return issues;
}
...
export function buildApprovedFilePlanSection(manifest?: FileManifest | null): string {
  ...
  const topologyLines = manifest.verifiedTopology
    ? manifest.verifiedTopology.edges.map((edge) => {
        const source = manifest.verifiedTopology!.nodes.find((node) => node.id === edge.sourceId)!;
        const target = manifest.verifiedTopology!.nodes.find((node) => node.id === edge.targetId)!;
        return `- ${source.path} ${edge.relation} ${target.path}${edge.canonicalSpecifier ? ` via ${edge.canonicalSpecifier}` : ""}`;
      })
    : [];
...
${topologyLines.length > 0 ? `VERIFIED PROSPECTIVE TOPOLOGY (AUTHORITY = 0):\n${topologyLines.join("\n")}\nUse these exact canonical dependency targets and module specifiers. Do not reconstruct topology from prose.` : ""}
...
══════════════════════════════════════════════════════════`;
    ...
    const topologyIssues = findVerifiedTopologyIssues(effectiveResolutionSourceMap, changes, approvedManifest);
    if (topologyIssues.length > 0) {
      throw new Error(`[GENERATED_TOPOLOGY_INVALID] ${topologyIssues.join("; ")}`);
    }
